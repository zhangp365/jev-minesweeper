...
function buildPrompt({ board, candidates, knownMines, level, step }) {
  ...
  const candidateLines = candidates.map((cell) => {
    const key = `(${cell.x},${cell.y})`;
    const tags = [];
    if (cell.forcedSafe) tags.push("由数字约束确定为安全");
    if (knownMines.has(`${cell.x},${cell.y}`)) tags.push("确定为雷（不会出现在候选中）");
    return `${key} 邻域: ${neighborSummary(board, cell.x, cell.y, knownMines)}${tags.length ? `；${tags.join("；")}` : ""}`;
  }).join("\n");
  return [
    ...
    "",
    "推理要求：",
    "1. 先做确定性推理：若某候选不可能有雷，优先选它。",
    "2. 无法确定时，结合剩余雷数评估每个候选的风险，选风险最低的。", "",
